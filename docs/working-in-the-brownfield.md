...
If using Factory CLI or Claude Code, use the architect to run the `*document-project` task to systematically produce key artifacts for your codebase (Factory: run `/use-architect` then request `*document-project`).
...
1. In your IDE (or Factory CLI), activate the architect and run `*document-project` (Factory: `/use-architect` → `*document-project`). Ensure you validate and direct the agent to produce the best possible documents for LLMs to understand your code base, and not include any misleading or unnecessary info.
