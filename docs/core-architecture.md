...
- **For IDEs**: Users interact with the agents directly via their markdown files in `bmad-core/agents/`. The IDE integration (for Cursor, Claude Code, etc.) knows how to call these agents. In Factory CLI, define commands under `<repo>/.factory/commands` (top-level files only) and custom droids under `<repo>/.factory/droids` (enable `enableCustomDroids` in settings) and invoke with slash commands.
